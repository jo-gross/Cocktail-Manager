...
import { TimeRangePicker, TimeRange, TimeRangePreset } from '@components/statistics/advanced/TimeRangePicker';
import { getStartOfWeek, getEndOfDay, getOrderedHourLabels, reorderHourDistribution } from '@lib/dateHelpers';
import { DAY_NAMES_SHORT_MONDAY_FIRST, DAY_ORDER_MONDAY_FIRST, reorderDaysMondayFirst } from '@lib/dayConstants';
import { StatCard } from '@components/statistics/advanced/StatCard';
import { TimeSeriesChart } from '@components/statistics/advanced/TimeSeriesChart';
import { DistributionChart } from '@components/statistics/advanced/DistributionChart';
import { StackedDistributionChart } from '@components/statistics/advanced/StackedDistributionChart';
import { AnalysisCocktailTable } from '@components/statistics/advanced/AnalysisCocktailTable';
import { ListDetailLayout } from '@components/statistics/advanced/ListDetailLayout';
import { CocktailList } from '@components/statistics/advanced/CocktailList';
import { TagList } from '@components/statistics/advanced/TagList';
import { IngredientList } from '@components/statistics/advanced/IngredientList';
import { SavedSetSelector } from '@components/statistics/advanced/SavedSetSelector';
...
import { AnalysisCocktailSelector } from '@components/statistics/advanced/AnalysisCocktailSelector';
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, Cell } from 'recharts';
import { CocktailStatisticItemFull } from '../../../../../../models/CocktailStatisticItemFull';
import '@lib/DateUtils';
import '@lib/StringUtils';
import { calculateAggregatedIngredientAmount, IngredientVolumeInfo, AmountWithUnit } from '@lib/CocktailRecipeCalculation';
...
const StatisticsAdvancedPage = () => {
  ...
  const { workspaceId, tab, startDate, endDate, preset, setId, analysisSetId } = router.query;
  const modalContext = useContext(ModalContext);
  const userContext = useContext(UserContext);

  // Initialize timeRange from URL params or defaults
  const getInitialTimeRange = useCallback((): TimeRange => {
      ...
      try {
        ...
        if (!isNaN(start.getTime()) && !isNaN(end.getTime())) {
          return {
            startDate: start,
            endDate: end,
            preset: (preset as TimeRangePreset) || 'custom',
          };
        }
      } catch (e) {
    ...
    return {
      startDate: weekStart,
      endDate: todayEnd,
      preset: (preset as TimeRangePreset) || 'thisWeek',
    };
  ...
  // Update timeRange when URL params change (only on mount or when URL actually changes)
  useEffect(() => {
    ...
    if (startDate && endDate && typeof startDate === 'string' && typeof endDate === 'string') {
      try {
        ...
        if (!isNaN(start.getTime()) && !isNaN(end.getTime())) {
          ...
          if (timeRange.startDate.getTime() !== newRange.startDate.getTime() || timeRange.endDate.getTime() !== newRange.endDate.getTime()) {
  ...
  }, [startDate, endDate, preset, router.isReady]);

  // Update selectedSetId when URL param changes
  useEffect(() => {
    if (!router.isReady) return;
    ...
    } else if (!setId && selectedSetId && activeTab === 'comparisons') {
      // Only clear if we're on the comparisons tab and no setId in URL
      setSelectedSetId(undefined);
    }
  }, [setId, router.isReady, activeTab]);

  // Update selectedAnalysisSetId when URL param changes
  useEffect(() => {
    if (!router.isReady || !workspaceId) return;

    if (analysisSetId && typeof analysisSetId === 'string') {
      if (selectedAnalysisSetId !== analysisSetId) {
        setSelectedAnalysisSetId(analysisSetId);
        // Load allCocktails first, then load set and select cocktails
        const loadSetWithCocktails = async () => {
          // Ensure allCocktails is loaded
          if (allCocktails.length === 0) {
            try {
              ...
              const cocktailsResponse = await fetch(`/api/workspaces/${workspaceId}/statistics/advanced/cocktails/all?${params}`);
              if (cocktailsResponse.ok) {
                const body = await cocktailsResponse.json();
                setAllCocktails(body.data);
              }
            } catch (error) {
              console.error('StatisticsAdvancedPage -> loadAllCocktails for set', error);
            }
          }
          // Load set and select cocktails in the list
          try {
            const setResponse = await fetch(`/api/workspaces/${workspaceId}/statistics/advanced/sets/${analysisSetId}`);
        ...
        loadSetWithCocktails();
      }
    } else if (!analysisSetId && selectedAnalysisSetId && activeTab === 'analysis') {
      // Only clear if we're on the analysis tab and no analysisSetId in URL
      setSelectedAnalysisSetId(undefined);
    }
  }, [analysisSetId, router.isReady, activeTab, workspaceId, allCocktails.length]);

  // Helper function to update URL parameter for setId
  const updateSetIdInUrl = useCallback(
    (setId: string | undefined, paramName: string = 'setId') => {
      if (typeof window === 'undefined' || !router.isReady) return;
      if (router.pathname !== window.location.pathname) return;
  ...
  // Helper function to update URL parameters for time range
  const updateTimeRangeInUrl = useCallback(
    (range: TimeRange, prefix: string = '') => {
      // Only update if router is ready and we're in browser context
      if (typeof window === 'undefined' || !router.isReady) return;

      // Check if we're currently navigating to avoid security errors
      if (router.pathname !== window.location.pathname) return;

      const startDateKey = prefix ? `${prefix}StartDate` : 'startDate';
      const endDateKey = prefix ? `${prefix}EndDate` : 'endDate';
      const presetKey = prefix ? `${prefix}Preset` : 'preset';

      const newQuery: Record<string, string | string[] | undefined> = {
        ...router.query,
        [startDateKey]: range.startDate.toISOString(),
        [endDateKey]: range.endDate.toISOString(),
        [presetKey]: range.preset || 'custom',
      };
      ...
      // Use requestAnimationFrame to defer URL update and avoid security errors
      requestAnimationFrame(() => {
        try {
          if (router.pathname === window.location.pathname) {
            router.replace(
          ...
          // Silently handle navigation errors (e.g., when navigating away)
          console.warn('Navigation error:', error);
        }
      });
    },
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="sticky top-16 z-10 bg-base-100 py-2">
          <div className="tabs-boxed tabs">
            {tabs.map((tab) => (
              <button key={tab.id} className={`tab ${activeTab === tab.id ? 'tab-active' : ''}`} onClick={() => handleTabChange(tab.id)}>
                {tab.label}
              </button>
            ))}
          </div>
        ...
        {activeTab === 'overview' && (
          <div className="flex flex-col gap-4">
            ...
            <div className="tabs tabs-bordered">
              <button className={`tab ${overviewPeriodTab === 'today' ? 'tab-active' : ''}`} onClick={() => setOverviewPeriodTab('today')}>
                Heute
              </button>
              <button className={`tab ${overviewPeriodTab === 'week' ? 'tab-active' : ''}`} onClick={() => setOverviewPeriodTab('week')}>
                Woche
              </button>
              <button className={`tab ${overviewPeriodTab === 'month' ? 'tab-active' : ''}`} onClick={() => setOverviewPeriodTab('month')}>
                Monat
              </button>
              <button className={`tab ${overviewPeriodTab === 'period' ? 'tab-active' : ''}`} onClick={() => setOverviewPeriodTab('period')}>
                Zeitraum
              </button>
              <button className={`tab ${overviewPeriodTab === 'allTime' ? 'tab-active' : ''}`} onClick={() => setOverviewPeriodTab('allTime')}>
                Allzeit
              </button>
            </div>
                ...
                <div className="w-full space-y-4">
                  ...
                  <SavedSetSelector
                    ...
                    onSelect={async (setIdValue, setType) => {
                      ...
                      updateSetIdInUrl(setIdValue, 'setId');
                    ...
                    <TagList
                      ...
                      onClear={() => {
                        ...
                        if (selectedSetId) {
                          ...
                          setOriginalComparisonSetItems(new Set());
                          setOriginalComparisonSetLogic('AND');
                          updateSetIdInUrl(undefined, 'setId');
                        }
                      }}
                ...
                <div className="w-full space-y-4">
                  ...
                  <SavedSetSelector
                    ...
                    onSelect={async (setId) => {
                      ...
                      updateSetIdInUrl(setId, 'analysisSetId');
                      if (setId) {
                  ...
                  <AnalysisCocktailSelector
                    ...
                    onClear={() => {
                      ...
                      if (selectedAnalysisSetId) {
                        setSelectedAnalysisSetId(undefined);
                        setOriginalAnalysisSetItems(new Set());
                        updateSetIdInUrl(undefined, 'analysisSetId');
                      }
                    }}
                  />
                </div>
