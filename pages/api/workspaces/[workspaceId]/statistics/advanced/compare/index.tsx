...
const legacyHandler = withHttpMethods({
  [HTTPMethod.GET]: withWorkspacePermission([Role.USER], Permission.STATISTICS_READ, async (req: NextApiRequest, res: NextApiResponse, user, workspace) => {
    const { type, items, logic, startDate, endDate } = req.query;
    ...
    const comparisonLogic = (logic as 'AND' | 'OR') || 'AND';
    ...
    // Get cocktails matching the items
    ...
    if (type === 'TAG_SET') {
      const cocktails = await prisma.cocktailRecipe.findMany({
        where: {
          workspaceId: workspace.id,
          tags: comparisonLogic === 'AND' ? { hasEvery: itemArray as string[] } : { hasSome: itemArray as string[] },
        },
        select: { id: true },
      });
      ...
      if (comparisonLogic === 'AND') {
        // For AND: cocktail must contain ALL ingredients
        // First, get all cocktails with their ingredients
        const cocktails = await prisma.cocktailRecipe.findMany({
          where: {
            workspaceId: workspace.id,
          },
        ...
        // Filter cocktails that contain ALL required ingredients
        ...
        // For OR: cocktail must contain at least one ingredient
        const cocktails = await prisma.cocktailRecipe.findMany({
          where: {
            workspaceId: workspace.id,
            steps: {
    ...
    // Get total statistics for percentage (needed even if no matching cocktails)
...
export default withDeprecation({ successor: '/api/v1/workspaces/{workspaceId}/statistics/advanced/compare' }, legacyHandler);
