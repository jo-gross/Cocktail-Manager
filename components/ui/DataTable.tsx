...
export function DataTable({ className, toolbar, children, emptyState, maxHeight = 'max-h-[70vh]', ...props }: DataTableProps) {
  return (
    <div className={cn('flex flex-col gap-3', className)} {...props}>
      ...
      <DataTableContext.Provider value={true}>
        <div className="overflow-hidden rounded-xl border border-base-300/60">
          <div className={cn('overflow-x-auto overflow-y-auto', maxHeight)}>{children}</div>
          {emptyState}
        </div>
...
export function TableImageCell({ hasImage = false, onImageClick, className, children, ...props }: TableImageCellProps) {
  return (
    <TableCell className={cn('w-0 p-2', className)} {...props}>
      <div
