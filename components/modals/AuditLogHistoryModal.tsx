...
const LONG_TEXT_FIELDS = ['Beschreibung', 'Zubereitung', 'Geschichte'];
...
export function AuditLogHistoryModal({ entityType, entityId, entityName }: AuditLogHistoryModalProps) {
  ...
  const renderSteps = (log: AuditLog, changes: any[]) => {
    const snapshot = log.snapshot;
    const steps: Record<string, any> = snapshot?.Schritte ?? {};
    const changedPaths = buildChangedPaths(changes);

    // Collect removed steps (kind='D' with path ['Schritte', stepKey])
    const removedSteps: Map<string, any> = new Map();
    changes.forEach((c: any) => {
      if (c.kind === 'D' && c.path?.length === 2 && c.path[0] === 'Schritte') {
        removedSteps.set(c.path[1], c.lhs);
      }
    });

    // Detect newly added steps (kind='N' with path ['Schritte', stepKey])
    const addedSteps = new Set<string>();
    changes.forEach((c: any) => {
      if (c.kind === 'N' && c.path?.length === 2 && c.path[0] === 'Schritte') {
    ...
    const hasPositionChange = changes.some((c: any) => c.path?.[0] === 'Schritte' && c.path?.[2] === 'Position' && c.kind === 'E');
      ...
      <div className="overflow-hidden rounded border bg-base-100">
        ...
        <div className="flex flex-col p-2">
          ...
          {Array.from(removedSteps.entries()).map(([stepKey, stepData]) => (
            <div key={`removed-${stepKey}`} className="mb-2 border-b border-base-200 pb-2 opacity-60 last:mb-0 last:border-b-0 last:pb-0">
              <div className="font-bold text-error line-through">
                − {t(stepData?.Aktion || stepKey)}
                {stepData?.Optional === 'Ja' ? ' (optional)' : ''}
              </div>
              {stepData?.Zutaten &&
                Object.entries(stepData.Zutaten).map(([ingName, ing]: [string, any]) => (
                  <div key={ingName} className="flex flex-row gap-2 pl-2 text-sm text-error/70 line-through">
                    <span>{ing?.Menge?.toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 2 }) ?? ''}</span>
                    <span>{ing?.Einheit ? t(ing.Einheit) : ''}</span>
                    <span>
                      {ingName}
                      {ing?.Optional === 'Ja' ? ' (optional)' : ''}
                    </span>
                  </div>
                ))}
            </div>
          ...
          {sortedSteps.map(([stepKey, stepData]) => {
            ...
            const actionName = stepData?.Aktion || stepKey;
            const actionChange = changedPaths.get(`Schritte.${stepKey}.Aktion`);
            const ingredientsMap: Record<string, any> = stepData?.Zutaten ?? {};
            ...
            return (
              ...
              >
                ...
                <div className={`flex flex-wrap items-center gap-1.5 font-bold ${isNew ? 'text-success' : ''}`}>
                  ...
                  {/* Optional status */}
                  {(() => {
                    const optChange = changedPaths.get(`Schritte.${stepKey}.Optional`);
                    if (optChange) {
                      if (optChange.kind === 'N' || (optChange.kind === 'E' && stepData?.Optional === 'Ja')) {
                        return <span className="badge badge-success badge-outline badge-xs">+ optional</span>;
                      }
                      if (optChange.kind === 'D' || (optChange.kind === 'E' && stepData?.Optional !== 'Ja')) {
                        return <span className="badge badge-error badge-outline badge-xs line-through">optional</span>;
                      }
                    }
                    return stepData?.Optional === 'Ja' ? <span className="text-xs font-normal text-base-content/50">(optional)</span> : null;
                  })()}
                </div>
                {/* Ingredient order change indicator */}
                {changes.some(
                  (c: any) =>
                    c.kind === 'E' && c.path?.[0] === 'Schritte' && c.path?.[1] === stepKey && c.path?.[2] === 'Zutaten' && c.path?.[4] === 'Position',
                ) && <div className="pl-2 text-xs italic text-info">Reihenfolge der Zutaten wurde geändert</div>}
                {/* Ingredients */}
                {sortedIngredients.map(([ingName, ing]) => {
                  const ingBase = `Schritte.${stepKey}.Zutaten.${ingName}`;
                  const amountChange = changedPaths.get(`${ingBase}.Menge`);
                  const unitChange = changedPaths.get(`${ingBase}.Einheit`);
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Optional status */}
                      {optionalChange ? (
                        optionalChange.kind === 'N' || (optionalChange.kind === 'E' && ing?.Optional === 'Ja') ? (
                          <span className="badge badge-success badge-outline badge-xs">+ optional</span>
                        ) : optionalChange.kind === 'D' || (optionalChange.kind === 'E' && ing?.Optional !== 'Ja') ? (
                          <span className="badge badge-error badge-outline badge-xs line-through">optional</span>
                        ) : null
                      ) : (
                        ing?.Optional === 'Ja' && <span className="text-xs text-base-content/50">(optional)</span>
                      )}
                    </div>
                  );
                })}
                ...
                {changes
                  .filter(
                    (c: any) => c.kind === 'D' && c.path?.[0] === 'Schritte' && c.path?.[1] === stepKey && c.path?.[2] === 'Zutaten' && c.path?.length === 4,
                  )
                  .map((c: any) => {
                    ...
                    return (
                      <div key={`removed-ing-${removedIngName}`} className="flex flex-row items-baseline gap-1 pl-2 text-sm text-error/70 line-through">
                        <span>−</span>
                        <span>{removedIng?.Menge?.toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 2 }) ?? ''}</span>
                        <span>{removedIng?.Einheit ? t(removedIng.Einheit) : ''}</span>
                        <span>{removedIngName}</span>
                        {removedIng?.Optional === 'Ja' && <span>(optional)</span>}
                      </div>
                    );
                  })}
              </div>
  ...
  const renderGarnishes = (log: AuditLog, changes: any[]) => {
    const snapshot = log.snapshot;
    const garnishes: Record<string, any> = snapshot?.Garnituren ?? {};
    const changedPaths = buildChangedPaths(changes);

    // Collect removed garnishes
    const removedGarnishes: Map<string, any> = new Map();
    changes.forEach((c: any) => {
      if (c.kind === 'D' && c.path?.length === 2 && c.path[0] === 'Garnituren') {
    ...
    changes.forEach((c: any) => {
      if (c.kind === 'N' && c.path?.length === 2 && c.path[0] === 'Garnituren') {
        addedGarnishes.add(c.path[1]);
      }
    });

    // Position change?
    const hasPositionChange = changes.some((c: any) => c.path?.[0] === 'Garnituren' && c.path?.[2] === 'Position' && c.kind === 'E');
        ...
        <div className="flex flex-col gap-1.5 p-2">
          ...
          {Array.from(removedGarnishes.entries()).map(([name, data]) => (
            <div key={`removed-${name}`} className="flex flex-wrap items-center gap-1.5 pl-2 text-sm text-error/70 line-through">
              <span>−</span>
              {data?.Alternative === 'Ja' && <span className="font-bold">oder</span>}
              <span>{name}</span>
              {data?.Optional === 'Ja' && <span>(optional)</span>}
              {data?.Notiz && <span className="text-base-content/50">– {data.Notiz}</span>}
            </div>
          ))}
          {/* Current */}
          {sortedGarnishes.map(([name, data]) => {
            const isNew = addedGarnishes.has(name);
            const gBase = `Garnituren.${name}`;
            const optionalChange = changedPaths.get(`${gBase}.Optional`);
            const alternativeChange = changedPaths.get(`${gBase}.Alternative`);
            const noteChange = changedPaths.get(`${gBase}.Notiz`);
            ...
            return (
              <div key={name} className={`pl-2 text-sm ${isNew ? 'font-medium text-success' : ''}`}>
                <div className="flex flex-wrap items-center gap-1.5">
                  ...
                  {/* Alternative status */}
                  {alternativeChange ? (
                    alternativeChange.kind === 'N' || (alternativeChange.kind === 'E' && data?.Alternative === 'Ja') ? (
                      <span className="badge badge-success badge-outline badge-xs">+ oder</span>
                    ) : alternativeChange.kind === 'D' || (alternativeChange.kind === 'E' && data?.Alternative !== 'Ja') ? (
                  ...
                  {/* Optional status */}
                  {optionalChange ? (
                    optionalChange.kind === 'N' || (optionalChange.kind === 'E' && data?.Optional === 'Ja') ? (
                      <span className="badge badge-success badge-outline badge-xs">+ optional</span>
                    ) : optionalChange.kind === 'D' || (optionalChange.kind === 'E' && data?.Optional !== 'Ja') ? (
                      <span className="badge badge-error badge-outline badge-xs line-through">optional</span>
                    ) : null
                  ) : (
                    data?.Optional === 'Ja' && <span className="text-xs text-base-content/50">(optional)</span>
                  )}
                </div>
                {/* Note change */}
                {noteChange && !isNew ? (
                  ...
                  data?.Notiz && !isNew && <div className="mt-0.5 pl-4 text-xs text-base-content/50">Notiz: {data.Notiz}</div>
                )}
                {isNew && data?.Notiz && <div className="mt-0.5 pl-4 text-xs">Notiz: {data.Notiz}</div>}
              </div>
            );
          })}
        </div>
  ...
  const renderGeneralChanges = (group: string, changes: any[]) => {
    ...
    const isIce = group === 'Eis';

    const renderedChanges = changes
      .map((change: any, index: number) => {
        const subPath = change.path?.slice(1).join(' › ') || '';
        ...
        const lhsDisplay = isIce && !lhsUndef ? t(String(lhsRaw)) : !lhsUndef ? String(lhsRaw) : '';
        const rhsDisplay = isIce && !rhsUndef ? t(String(rhsRaw)) : !rhsUndef ? String(rhsRaw) : '';
    ...
    return (
      <div className="overflow-hidden rounded border bg-base-100">
        <div className="border-b bg-base-200 px-3 py-1 font-bold capitalize text-base-content">{group}</div>
  ...
  const renderDiff = (log: AuditLog) => {
    ...
    if (log.action === 'UPDATE' && Array.isArray(log.changes)) {
      ...
      log.changes.forEach((change: any) => {
        const root = change.path?.[0] || 'Allgemein';
      ...
      return (
        <div className="flex flex-col gap-3 text-sm">
          {Object.entries(groupedChanges).map(([group, changes]) => {
            ...
            if (group === 'Schritte') return <React.Fragment key={group}>{renderSteps(log, changes)}</React.Fragment>;
            if (group === 'Garnituren') return <React.Fragment key={group}>{renderGarnishes(log, changes)}</React.Fragment>;
            if (group === 'Bild') return <React.Fragment key={group}>{renderImage(changes)}</React.Fragment>;
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {log.snapshot && (
                        <button
                          ...
                          onClick={() => {
                            const blob = new Blob([JSON.stringify(log.snapshot, null, 2)], { type: 'application/json' });
