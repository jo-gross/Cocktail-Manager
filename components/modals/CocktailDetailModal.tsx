    ...
    <div className={"card-body bg-base-100"}>
      ...
      <div className={"grid grid-cols-2 gap-4"}>
        <div className={"col-span-2 flex"}>
          {props.cocktail.tags.map((tag, index) => (<div className={"m-1 badge badge-primary"}>{tag}</div>))}
        </div>
        ...
        <div className={"col-span-1"}>
          ...
          <img src={props.cocktail.glass.image} alt={"Glas"} className={"w-16 h-16"} />
        </div>
        <div className={"col-span-1"}>
          Eis: {props.cocktail.glassWithIce}
        </div>
        <div className={"col-span-2 space-y-2"}>
            ...
            {props.cocktail.steps.map((step, index) => (
              ...
              >
                ...
                {step.ingredients.map((ingredient, index) => (
                  <div key={"cocktail-details-step-ingredient-" + ingredient.id}
                       className={"pl-2"}>
                    <div className={"flex-1"}>
                      <div className={"flex flex-row space-x-2"}>
                        <div className={"font-bold"}>{ingredient.amount}{ingredient.unit}</div>
                        <span>{ingredient.ingredient.name}</span>
                        {ingredient.ingredient.image != undefined ? <img src={ingredient.ingredient.image} alt={""} className={"object-cover h-16 avatar"} /> : <></>}
                      </div>
                    </div>
                    <div>
                      {ingredient.ingredient.tags?.map((tag, index) => (<div className={"m-1 badge badge-primary"}>{tag}</div>))}
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div
            className={"col-span-2 border-2 border-base-300 rounded-lg p-2 space-y-2 flex flex-row space-x-2 items-center"}
          >
            <span className={"font-bold text-xl flex-1"}>{props.cocktail.decoration.name}</span>
            {props.cocktail.decoration.image != undefined ? <img src={props.cocktail.decoration.image} alt={"Deko"} className={"object-cover h-16 avatar"} /> : <></>}
          </div>
        </div>
      </div>
    </div>
